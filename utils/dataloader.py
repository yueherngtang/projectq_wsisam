...
def create_dataloaders(name_im_gt_list, my_transforms=[], batch_size=1, training=True):
    gos_dataloaders = []
    gos_datasets = []
    ...
    if training:
        ...
        dataloader = DataLoader(gos_dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=num_workers_)
...
class RandomColor(object):
    ...
    def __call__(self,sample):
        ...
        if random.random() >= self.prob:
            ...
            image = color_jitter(image.to(dtype=torch.uint8))
